/**
 * Returns the canonical base URL for the current deployment.
 *
 * Priority:
 *   1. NEXT_PUBLIC_SITE_URL  — set this in the Vercel dashboard when you
 *                              attach a custom domain (e.g. https://hrs.com)
 *   2. VERCEL_URL            — Vercel injects this automatically on every
 *                              deployment (preview + production)
 *   3. localhost fallback    — local dev
 */
export function getBaseUrl() {
  if (process.env.NEXT_PUBLIC_SITE_URL) {
    return process.env.NEXT_PUBLIC_SITE_URL.replace(/\/$/, '');
  }
  if (process.env.VERCEL_URL) {
    return `https://${process.env.VERCEL_URL}`;
  }
  return 'http://localhost:3001';
}

/**
 * Returns the public contact email used by the CTA buttons and structured data.
 * Set NEXT_PUBLIC_CONTACT_EMAIL in the environment (see .env) to change it.
 */
export function getContactEmail() {
  return process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'info@hrsrobot.co.uk';
}

/**
 * GA4 Measurement ID (looks like "G-XXXXXXXXXX"). Empty until one is set, in
 * which case no Google Analytics tag is loaded at all — so local dev and
 * preview deployments stay out of the production property.
 */
export function getGaId() {
  return process.env.NEXT_PUBLIC_GA_ID || '';
}

/**
 * Microsoft Clarity project ID (a short alphanumeric string). Same deal as
 * getGaId — no ID, no tag.
 */
export function getClarityId() {
  return process.env.NEXT_PUBLIC_CLARITY_ID || '';
}
